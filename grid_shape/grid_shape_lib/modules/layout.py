...
class Layout:
    def __init__(
        ...
        self.radius_contained = radius_contained
        ...
        self.area = np.int32(trihex_steps[0])**2 *3*np.sqrt(3)/2 * self.nb_hex  ## area of the simulation
        if self.radius_contained > 0:
            self.area = np.int32(self.radius_contained)**2 *3*np.sqrt(3)/2
        ...
        [
            ua.create_ev_select(
                ...
                radius_contained=self.radius_contained
            )
        ...
        self.delta_theta = self.zenith_bins_limits[1:] - \
            self.zenith_bins_limits[:-1]   ## this is in degrees

        self.delta_zenith = (self.zenith_bins_limits[1:] - self.zenith_bins_limits[:-1]) /180*np.pi ## this in in radian
    ...
    def plot_layout(self, fig=1, do_title=True):
        ...
        plt.axis('equal')
        if do_title:
            plt.title('{}, {} ant.'.format(self.mask_name, self.mask.sum()))
        plt.tight_layout()
        plt.savefig(
    ...
    def compute_trig_efficiency(self):
        ...
        num_trigged = np.zeros((n_energy_bins, n_zenith_bins))
        num_triggable = np.zeros((n_energy_bins, n_zenith_bins))
        ...
        for i_ener in range(n_energy_bins):
            for i_zen in range(n_zenith_bins):
                ...
                if len(ind[0]) == 0:
                    ...
                    num_trigged[i_ener, i_zen] = sum(
                        self.ev_select[ind[0],4]
                    )
                    num_triggable[i_ener, i_zen] = np.size(self.ev_select[ind[0],4])
        ...
        self.num_trigged = num_trigged
        self.num_triggable = num_triggable

    def compute_detection_rate(self):
        """
        compute the detection rate
        number of events per day per m2 TBModified
        """
        ...
        for i_ener, ener in enumerate(self.energy_bins_centers):
            for i_zen, zen in enumerate(self.zenith_bins_centers):
                differential_rate[i_ener, i_zen] = (
                        ...
                        self.area * np.cos(zen*np.pi/180) *
                ...
                detection_rate[i_ener, i_zen] = (
                        self.trig_efficiency[i_ener,i_zen] * 
                        diff_spec.tale_diff_flux(ener*1e18) * 
                        self.delta_energy[i_ener] *1e18 * np.cos(zen/180*np.pi) * 2*np.pi * self.delta_zenith[i_zen] *
                        self.area * np.sin(zen*np.pi/180) *
                        3600 * 24 # !! finally we divide by the area here
                    )
        ...
        self.detection_rate_summed_vs_energy = self.detection_rate.sum(axis=1)
        self.detection_rate_summed_vs_zenith = self.detection_rate.sum(axis=0)
    ...
    def plot_2D_differential_rate(self):
        ...
        cbar.ax.set_ylabel('$ [day^{-1} PeV^{-1} sr^{-1}]$')
        plt.savefig(
    ...
    def plot_2D_num_trigged(self):
        ...
        levels = [1, 5, 10, 20]
        cs2 = plt.contour(xx,yy,
            self.num_trigged.T, levels=levels, colors = 'k'
        )
        ax.clabel(cs2, inline=1, fontsize=10,colors="k", fmt= '%1.2f' )
        ...
        #cbar.ax.set_ylabel('$$')
        plt.savefig(
            os.path.join(self.plot_path, "num_trigged_{}.png".format(self.plot_suffix))
        )
    
    def plot_2D_num_triggable(self):
        fig, ax = plt.subplots()
        plt.clf()
        CS = plt.pcolor(
            np.log10(1e18*self.energy_bins_limits),
            self.zenith_bins_limits,
            self.num_triggable.T,
            norm=LogNorm( vmin=1, vmax=100)
        )
        x = np.log10(1e18*self.energy_bins_centers)
        y = self.zenith_bins_centers
        ...
        levels = [1, 10, 50, 100]
        cs2 = plt.contour(xx,yy,
            self.num_triggable.T, levels=levels, colors = 'k'
        )
        ax.clabel(cs2, inline=1, fontsize=10,colors="k", fmt= '%1.2f' )
        ...
        #cbar.ax.set_ylabel('$$')
        plt.savefig(
            os.path.join(self.plot_path, "num_triggable_{}.png".format(self.plot_suffix))
        )
    ...
    def plot_2D_detection_rate(self):
        ...
        ## plot 1D detection rates
        plt.figure()
        plt.plot(self.energy_bins_centers, self.detection_rate_summed_vs_energy)
        plt.ylabel('number of events [day'+"$^{-1}$]")
        plt.xlabel('Energy [EeV]')
        plt.xscale('log')
        plt.yscale('log')
        plt.savefig(
            os.path.join(self.plot_path, "diff_ev_rate_perday_vs_energy_{}.png".format(self.plot_suffix))
        )
        plt.figure()
        plt.plot(self.zenith_bins_centers, self.detection_rate_summed_vs_zenith)
        plt.ylabel('number of events [day'+"$^{-1}$]")
        plt.xlabel('Zenith [deg]')
        plt.xscale('log')
        plt.yscale('log')
        plt.savefig(
            os.path.join(self.plot_path, "diff_ev_rate_perday_vs_zenith_{}.png".format(self.plot_suffix))
        )


    def plot_mean_shower_area(self, do_title=True):
        self.mean_shower_area = self.area/self.mask.sum() * self.mean_n_trig / 1e6
        self.std_shower_area = self.area/self.mask.sum() * np.sqrt(self.var_n_trig)
        fig, ax = plt.subplots()
        CS = plt.pcolor(
            np.log10(1e18*self.energy_bins_limits),
            self.zenith_bins_limits,
            self.mean_shower_area.T,
            norm=LogNorm(vmin=0.01, vmax=np.nanmax(self.mean_shower_area)) 
        )
        x = np.log10(1e18*self.energy_bins_centers)
        y = self.zenith_bins_centers
        xx,yy = np.meshgrid(x,y)
        levels = [0.01, 0.03, 0.1, 0.3, 1, 3, 10, 30, 100]
        cs2 = plt.contour(xx,yy,
            self.mean_shower_area.T, levels=levels, colors = 'k'
        )
        ax.clabel(cs2, inline=3, fontsize=10,colors="k", fmt= '%2.1f' )
        plt.xlabel('log E/eV', fontsize=18)
        plt.ylabel('Zenith [deg]', fontsize=18)
        if do_title:
            plt.title('Mean shower area, {}'.format(self.plot_suffix))
        else:
            plt.title('Mean shower area [km x km]')
        cbar = plt.colorbar(CS)
        plt.tight_layout()
        ...
        plt.savefig(
            os.path.join(
                self.plot_path,
                "mean_shower_area_{}.png".format(self.plot_suffix)
            )
        )


    def plot_mean_sep_antenna(self, do_title=True):
        self.mean_sep_antenna = np.sqrt(self.mean_shower_area/self.n_trig_thres)

        fig, ax = plt.subplots()
        CS = plt.pcolor(
            np.log10(1e18*self.energy_bins_limits),
            self.zenith_bins_limits,
            self.mean_sep_antenna.T,
            norm=LogNorm(vmin=0.1/self.n_trig_thres, vmax=np.nanmax(self.mean_sep_antenna)) 
        )
        x = np.log10(1e18*self.energy_bins_centers)
        y = self.zenith_bins_centers
        xx,yy = np.meshgrid(x,y)
        levels = [0.05, 0.1, 0.2, 0.5, 1, 2, 3, 10, 30, 100]
        cs2 = plt.contour(xx,yy,
            self.mean_sep_antenna.T, levels=levels, colors = 'k'
        )
        ax.clabel(cs2, inline=3, fontsize=10,colors="k", fmt= '%2.1f' )
        plt.xlabel('log E/eV', fontsize=15)
        plt.ylabel('Zenith [deg]', fontsize=15)
        if do_title:
            plt.title('Mean antenna separation, {}'.format(self.plot_suffix))
        else:
            plt.title('Mean antenna separation [km]')
        cbar = plt.colorbar(CS)
        plt.tight_layout()
        #cbar.ax.set_ylabel('$ [day^{-1} km^{-2} PeV^{-1} sr^{-1}]$')
        plt.savefig(
            os.path.join(
                self.plot_path,
                "mean_antenna_sep_{}.png".format(self.plot_suffix)
            )
        )
    ...
    def plot_mean_n_trig(self, do_title=True):
        fig, ax = plt.subplots()
        CS = plt.pcolor(
            np.log10(1e18*self.energy_bins_limits),
            self.zenith_bins_limits,
        ...
        if do_title:
            ...
            plt.title('Mean number of trigged antennas')
        cbar = plt.colorbar(CS)
        #cbar.ax.set_ylabel('$ [day^{-1} km^{-2} PeV^{-1} sr^{-1}]$')
        plt.tight_layout()
        plt.savefig(
        ...
        CS = plt.pcolor(
            np.log10(1e18*self.energy_bins_limits),
            self.zenith_bins_limits,
            np.sqrt(self.var_n_trig).T,
            norm=LogNorm(vmin=1, vmax=np.nanmax(np.sqrt(self.var_n_trig))) 
        )
        x = np.log10(1e18*self.energy_bins_centers)
        y = self.zenith_bins_centers
        xx,yy = np.meshgrid(x,y)
        levels = [2, 5, 10, 20, 50, 100, 200, 500]
        cs2 = plt.contour(xx,yy,
            np.sqrt(self.var_n_trig).T, levels=levels, colors = 'k'
        )
        ax.clabel(cs2, inline=1, fontsize=10,colors="k", fmt= '%2d' )
        plt.xlabel('log E/eV')
        plt.ylabel('Zenith [deg]')
        plt.title('std N trig, {}'.format(self.plot_suffix))
        cbar = plt.colorbar(CS)
        #cbar.ax.set_ylabel('$ [day^{-1} km^{-2} PeV^{-1} sr^{-1}]$')
        plt.savefig(
            os.path.join(
                self.plot_path,
                "std_n_trig_{}.png".format(self.plot_suffix)
            )
        )
    ...
    def make_all_plots(self, do_title=True):
      
        self.plot_2D_detection_rate()
        self.plot_2D_differential_rate()
        self.plot_mean_n_trig(do_title=do_title)
        self.plot_mean_shower_area(do_title=do_title)
        self.plot_mean_sep_antenna(do_title=do_title)
        self.plot_2D_trig_efficiency()
        self.plot_2D_num_triggable()
        self.plot_2D_num_trigged()
        self.plot_trig_efficiency()
        self.plot_layout(do_title=do_title)
        #self.plot_2d_efficiency(5, 5)
